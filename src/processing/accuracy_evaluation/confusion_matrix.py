...
def np_to_py(val):
    """递归把所有 numpy 类型转为原生 python 类型，包括字典的key和value"""
    import numpy as np
    if isinstance(val, dict):
        # 关键：key强转为str（或int），防止 int64 问题
        return {str(np_to_py(k)): np_to_py(v) for k, v in val.items()}
    if isinstance(val, list):
        return [np_to_py(x) for x in val]
...
def evaluate_from_samples_file(samples_file: str,
                               output_dir: str = "results",
                               label_names: dict = None,
                               formats: list = ['png', 'csv', 'json', 'txt'],
                               prefix: str = "confusion_matrix"):
    """
    以xlsx/csv点表为唯一入口，自动完成全流程分类精度评估
    """
    y_true, y_pred = load_samples_from_file(samples_file)
    labels = sorted(list(set(y_true) | set(y_pred)))
    analyzer = ConfusionMatrixAnalyzer(
        y_true, y_pred, labels=labels, label_names=label_names
    )
    analyzer.save_results(output_dir, formats=formats, prefix=prefix)
    return analyzer.metrics if analyzer.metrics is not None else analyzer.compute_metrics()
